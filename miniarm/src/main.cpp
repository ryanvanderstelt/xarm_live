...
bool at_target(uint8_t targets[5])
{
  for (int i = 0; i < 5; i++)
  {
    ...
    if (abs(current_angles[i] - targets[i]) > 5)
      return false;
  }
  return true;
}
...
void update_positions(uint8_t targets[5])
{
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    {
      current_angles[i]+=5; // Move joint clockwise
    }
    else if (current_angles[i] > targets[i])
    {
      current_angles[i]-=5; // Move joint counter-clockwise
    }
...
void dynamic_move_to_position(uint8_t targets[5])
{

  // Apply safety limits to target positions
  for (int i = 0; i < 5; i++)
  {
    targets[i] = constrain(targets[i],          // Raw preset value
                           JOINT_LIMITS[i][0],  // Minimum angle
                           JOINT_LIMITS[i][1]); // Maximum angle
  }
...
void setup()
{
  // Initialize serial communication at the same baud rate as the Python script
  Serial.begin(115200);
...
void loop()
{
  if (Serial.available() > 0)
  {
    ...
    // Print the received string to the Arduino Serial Monitor
    Serial.print("Received in Arduino: ");
    ...
    case 'b':
      joint = 4;
      if (incomingString[1] == 'l')
      {
        angle = current_angles[joint] - diff;
      }
      else
      {
        angle = current_angles[joint] + diff;
    ...
    uint8_t targets[5] = {current_angles[0], current_angles[1], current_angles[2], current_angles[3], current_angles[4]};
    if (joint != -1)
    {
      targets[joint] = angle;
      dynamic_move_to_position(targets);
    }
  }
}
